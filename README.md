...
推荐使用下面这个命令：先下载到 VPS 本地，再用 `bash` 运行；如果检测到中国大陆网络，会自动改走 GitHub 代理。不要用 `curl ... | bash` 运行交互菜单，因为管道可能占用标准输入，导致上下键菜单显示不完整或无法选择。

```bash
NBO_ARGS=""
NBO_VERSION="2026.06.18.4"
NBO_RAW="https://raw.githubusercontent.com/GHUNLIL/network-bbr-optimizer/main/bbr.sh"
NBO_PROXY_PREFIX="${GITHUB_PROXY_PREFIX:-https://gh-proxy.com/}"
NBO_PROXY="${NBO_PROXY_PREFIX%/}/${NBO_RAW}"
NBO_URL="${NBO_RAW}"
if curl -fsSL --connect-timeout 1 --max-time 2 https://www.cloudflare.com/cdn-cgi/trace 2>/dev/null | grep -q '^loc=CN$'; then
  NBO_URL="${NBO_PROXY}"
fi
TMP_BBR="/tmp/network-bbr-optimizer.sh"
curl -fL "${NBO_URL}?v=${NBO_VERSION}" -o "${TMP_BBR}" || curl -fL "${NBO_PROXY}?v=${NBO_VERSION}" -o "${TMP_BBR}"
chmod +x "${TMP_BBR}"
if [ "$(id -u)" -eq 0 ]; then
  bash "${TMP_BBR}" ${NBO_ARGS}
else
  sudo bash "${TMP_BBR}" ${NBO_ARGS}
fi
```
...
NBO_PROXY_PREFIX="${GITHUB_PROXY_PREFIX:-https://gh-proxy.com/}"
NBO_PROXY="${NBO_PROXY_PREFIX%/}/${NBO_RAW}"
curl -fL "${NBO_RAW}?v=2026.06.18.4" -o bbr.sh || curl -fL "${NBO_PROXY}?v=2026.06.18.4" -o bbr.sh
chmod +x bbr.sh
