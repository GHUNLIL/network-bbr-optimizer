...
- 应用新版配置时，会备份并移除 `/etc/sysctl.d` 中其它写入本脚本负责网络优化参数的 sysctl 文件，让 `/etc/sysctl.d/99-network-optimize.conf` 成为唯一主配置。匹配按 sysctl key 自动识别，不按旧文件名写死；`98-wgmimic-required.conf`、README、以及 disabled/bak/old/orig/save 文件会保留，回滚脚本会恢复被接管移除的文件。
